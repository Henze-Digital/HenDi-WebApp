...
<xsl:stylesheet 
    ...
    -->
	<xsl:variable name="linkableElements" as="xs:string+" select="('persName', 'rs', 'name', 'characterName', 'orgName', 'country', 'bloc', 'geogName', 'sic', 'del', 'add', 'subst', 'damage', 'choice', 'unclear', 'app', 'note', 'region', 'settlement', 'bibl')"/>
    
    <!--  *********************************************  -->
    <!--  *                  Templates                *  -->
    <!--  *********************************************  -->
	<xsl:template match="tei:persName | tei:author | tei:orgName | tei:country | tei:bloc | tei:geogName | mei:persName | tei:name | tei:region | tei:settlement | mei:settlement | mei:geogName | mei:corpName | mei:title[@codedval] | tei:placeName[@key] | tei:bibl[@key]" 
        mode="#all">
        <xsl:choose>
        ...
        <xsl:attribute name="href">
            <xsl:choose>
                <xsl:when test="starts-with(.,'hendi:')">
                    ...
                    <xsl:value-of select="concat(wega:createLinkToDoc(substring(., 7, 8), $lang), substring(., 13))"/>
                </xsl:when>
                <xsl:otherwise>
                    <xsl:value-of select="."/>
                </xsl:otherwise>
