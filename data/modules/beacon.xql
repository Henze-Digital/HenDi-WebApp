...
declare function beacon:new($type as xs:string) as xs:string {
    ...
    let $desc := 
        switch($type)
        case 'pnd' return '#DESCRIPTION: Personendatensätze von Henze-Digital'
        case 'gkd' return '#DESCRIPTION: Datensätze Organisationen/Körperschaften von Henze-Digital'
        case 'works' return '#DESCRIPTION: Werkdatensätze von Henze-Digital'
        default return ()
    let $feed := 
        switch($type)
        case 'pnd' return '#FEED: http://henze-digital.zenmem.de/pnd_beacon.txt'
        case 'gkd' return '#FEED: http://henze-digital.zenmem.de/gkd_beacon.txt'
        case 'works' return '#FEED: http://henze-digital.zenmem.de/works_beacon.txt'
        default return ()
    let $header := (
        '#FORMAT: BEACON',
        '#PREFIX: http://d-nb.info/gnd/',
        '#VERSION: 0.1',
        '#TARGET: https://henze-digital.zenmem.de/de/gnd/{ID}',
        $feed,
        '#CONTACT: Dennis Ried <dried [ at ] uni-paderborn.de>',
        '#INSTITUTION: Henze-Digital (HenDi)',
        $desc,
